...
def build_renaming_commands(src: Path, dst: Path, output_stream=sys.stdout):
    ...
    if not src or not dst:
        return
    if src == dst:
        logging.info("Identical names, no renaming needed for: %s", src.absolute())
        return
    if dst.exists():
        logging.warning("Renaming target already exists - do nothing! (%s)", dst.absolute())
        return
    if _is_os_windows():
...
def run(rootdir: Path, output_stream=sys.stdout):
    """Run the main job.
    ...
    if not rootdir.is_dir():
        raise NotADirectoryError("%s is not a directory!" % rootdir)
    ...
    for root, dirs, _ in os.walk(str(rootdir.resolve())):
        for dirname in dirs:
            ...
            for src in dirpath.glob("*.mkv"):
                ...
                build_renaming_commands(nfo_filepath, nfo_filepath_new, output_stream)
