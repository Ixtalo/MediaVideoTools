...
def __get_path_languages(filepath: Path, use_full_path: bool = True) -> set:
    assert len(filepath.parts) > 1, "filepath must have filename and parent directory!"
    ...
    path_languages = re.findall(r"\[([A-Z]{2})\]", str(dirpath) if use_full_path else str(dirname))
...
def get_track_languages_for_file(filepath: Path) -> set:
    ...
    if filepath.is_dir():
        raise IsADirectoryError("filepath must be a valid file path, not a directory!")
...
def get_track_languages_for_files(paths: list[Path]) -> set:
    ...
    for filepath in paths:
        ...
        if not track_languages:
            ...
            continue
        logging.debug("%s in '%s'", ",".join(list(track_languages)), filepath.name)
...
def scan(rootdir: Path, use_full_path: bool = True):
    ...
    for root, _, files in sorted(os.walk(rootdir.resolve())):
        ...
        root_languages = __get_path_languages(Path(root, "dummy"), use_full_path=use_full_path)
        ...
        if files_languages:
            ...
            if missing:
                logging.warning("%s missing in path: %s", ",".join(missing), relative_path)
                problems["missing_in_path"] = sorted(list(missing))
            toomuch = __get_toomuch_in_path(root_languages, files_languages)
            if toomuch:
                logging.warning("%s too much in path: %s", ",".join(toomuch), relative_path)
                problems["toomuch_in_path"] = sorted(list(toomuch))

        if problems:
            result[str(relative_path)] = problems
        else:
            logging.debug("OK: '%s', languages: %s", relative_path, ",".join(files_languages))
