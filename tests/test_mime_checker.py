...
from mediavideotools.mime_checker import is_mediafile, is_video, is_audio
...
def test_is_mediafile():
    ...
    assert is_mediafile(
        Path("./testdata/correct/SampleVideoFlv/sample_640x360_1sec.flv"))
    ...
    assert is_mediafile(
        Path("./testdata/correct/symlinks/SampleVideo_1280x720_1sec.mkv"))
...
def test_is_video():
    ...
    assert is_video(
        Path("./testdata/correct/Cool Run (1993) [EN]/subdir/cool.run.720p.bluray.hevc.x265.rmteam_cut.mkv"))
    assert is_video(Path(
        "./testdata/correct/Forrest Video (1994) [DE][EN]/Forrest.1994.German.AC3.DL.1080p.BluRay.x265-FURTUM_cut.mkv"))
    assert is_video(
    ...
    assert is_video(
        Path("./testdata/correct/SampleVideoFlv/sample_640x360_1sec.flv"))
    assert is_video(
        Path("./testdata/correct/Unicode-äöüß/SampleVideo_1280x720_1sec_äöüß.mkv"))
